...
- [ ] **Refactor `siga/client.py` para consumir `siga_selectors.yaml`** — substituir os locators "guess N alternativas" (linhas 45-97 de `client.py`) por `get_field("student_detail", "nome")` etc. Manter fallback para o guess-based path durante período de validação. Depende de inspeção humana do manifest gerado (18 screens em `procedures_data/siga_capture/latest/`).
