...
- [ ] **Fleet OOM com N ≥ ~5 sub-agentes carregando `multilingual-e5-large` concorrentemente** — smoke com `--limit 10` (9 emails para Tier 1) segfaultou em `Loading weights: 0/391` dentro do LangGraph Fleet. Cada sub-agente instancia seu próprio `SentenceTransformer` para RAG + Reflexion, e o modelo tem ~2 GB. Mitigações possíveis: (a) singleton compartilhado do encoder entre sub-agentes (thread-safe — `SentenceTransformer.encode` é thread-safe pra inferência), (b) reduzir `FLEET_BROWSER_POOL_SIZE` e serializar encoding, (c) lazy load com `pool_strategy="mean"` + 8-bit quantization. Workaround imediato: processar em batches menores (`--limit 5` ou `--limit 3`).
- [ ] **DSPy + Fleet thread conflict** — smoke 2026-04-22 `--langgraph --limit 3` revelou: `Fleet[...] sub-agent failed entirely: dspy.settings can only be changed by the thread that initially configured it.` 2 de 3 sub-agents crasharam; apenas 1 (o que ganhou a corrida e inicializou DSPy primeiro) classificou. Root cause: DSPy usa thread-local settings e `LangGraph.Send` dispara sub-agents em threads diferentes do pool. Workaround validado: `USE_DSPY=off` força LiteLLM direto (sem perda de qualidade detectada). Fix permanente: ou (a) inicializar DSPy no thread principal ANTES do `dispatch_tier1` e passar explicitamente via context, ou (b) usar `dspy.settings.context()` per-thread, ou (c) retry com `USE_DSPY=off` quando esse erro é detectado.
- [ ] **`AFLOW_TOPOLOGY=baseline` quebra com `RuntimeError: asyncio.run() cannot be called from a running event loop`** em `graph/nodes.py:977` (`consultar_siga` → `asyncio.run(_consult_siga_async(...))`). LangGraph invoca nodes dentro de event loop quando chamado via `graph.invoke()` a partir de função async (CLI `run_gmail_channel`). Default `fleet` não é afetado porque `process_one_email` já é sync. Fix: trocar `asyncio.run(...)` por `loop = asyncio.get_event_loop(); loop.run_until_complete(...)` com fallback, ou (melhor) tornar `consultar_siga` async e deixar o LangGraph await. Afeta também `consultar_sei` em `nodes.py` se seguir o mesmo padrão — auditar.
