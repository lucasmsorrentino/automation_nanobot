...
def _load_procedure_records(stable_id: str) -> list[dict[str, Any]]:
    ...
    return [
        r.model_dump() for r in all_records if r.email_hash == stable_id
    ]
...
def format_report(trace: DebugTrace) -> str:
    ...
    if trace.has_correction:
        lines.extend([
            ...
            f"- Notes: {trace.correction_notes}" if trace.correction_notes else "- Notes: (none)",
        ])
    else:
...
def main() -> None:
    ...
    for t in traces:
        status = "MISMATCH" if t.has_correction and t.corrected_categoria != t.pipeline_categoria else "ok"
