...
**SEI process lookup cascade** — `graph/nodes.py:consultar_sei` agora tenta em ordem: (1) nº SEI UFPR `23075.*` explícito no texto/anexos, (2) GRR extraído → `SEIClient.find_processes_by_grr` via `#txtPesquisaRapida` + parse de `table.pesquisaResultado`. Quando volta N>1 processos, `select_best_processo` desambigua em 2 fases: ano dominante (mais novo = ativo — regra do usuário) → desempate intra-ano por status/tipo/GRR/data. Roteiro completo em `TASKS.md §ATIVA`; caminho #2 (AE keyword) pendente de implementação. `SIGAClient._navigate_to_student` bumpa pagination pra 300 antes do filter (filter é client-side, default era 20/página e escondia alunos fora da 1ª página). `extract_sei_process_number` restrito a prefixo UFPR para ignorar IFPR/MEC.
