...
A specialized deployment automating bureaucratic email processing at UFPR (Universidade Federal do Paraná). Currently in **Marco II+III unified + Hybrid Memory (Tier 0 playbook)** phase.
...
- **`graph/`** — LangGraph StateGraph orchestrator (Marco II). Nodes: perceber (Gmail/OWA) → **tier0_lookup (Hybrid Memory)** → rag_retrieve (RAPTOR/flat + Reflexion, Tier 1 residual only) → classificar (DSPy/LiteLLM, merges Tier 0 + Tier 1) → rotear (confidence routing) → consultar_sei/consultar_siga (conditional, Estágios) → agir (save drafts) → registrar_procedimento. `state.EmailState.tier0_hits` carries the set of stable_ids resolved by Tier 0 so downstream nodes can skip them. SQLite checkpointing for fault tolerance.
