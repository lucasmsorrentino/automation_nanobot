...
#          via SEI_SELECTORS_PATH). Sprint 3 validação concluída em 23075.020970/2026-09
#          (2026-04-13), fix Ctrl+A+Delete aplicado a save_despacho_draft em 2026-04-14.
...
- **`siga/`** — SIGA (Sistema Integrado de Gestão Acadêmica) integration via Playwright. READ-ONLY: checks student status, enrollment, validates internship eligibility per SOUL.md section 11 rules. `browser.py` keeps only the SIGA-specific login form + `is_logged_in` selectors; generic Playwright context/session lifecycle is shared with SEI via `ufpr_automation/_session_browser.py`.
