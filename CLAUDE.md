...
> Last updated: 2026-05-02. Major recent changes: Refator das Ondas 1+2 mergeadas em `dev` (PR #4 + PR #5; -533 LOC + -1581 LOC); Onda 3 deletou DSPy completamente — `dspy_modules/`, `USE_DSPY` gate e `_classify_with_dspy` removidos, alias map de Categoria preservado em `core/models.py:normalize_categoria`. Path único agora é LiteLLM via `_classify_with_litellm`.
...
# Feedback — human corrections store (revisão e log do que o LLM acertou/errou)
...
python -m ufpr_automation.rag.advanced.raptor --max-levels 3             # custom depth
python -m ufpr_automation.rag.advanced.raptor --dry-run                  # cluster stats only
...
- **Tier 1 — RAG + LLM**: unchanged path (RAPTOR/flat retrieval + Reflexion → LiteLLM classifier). Only emails that Tier 0 missed (or had missing required fields, or are stale) ever reach this tier.
...
- **`core/`** — Domain model (`EmailData` with `AttachmentData` list, `EmailClassification` with confidence score). The `Categoria` Literal uses a hierarchical taxonomy with ` / ` as separator: `Estágios`, `Acadêmico / {Matrícula,Equivalência de Disciplinas,Aproveitamento de Disciplinas,Ajuste de Disciplinas}`, `Diplomação / {Diploma,Colação de Grau}`, `Extensão`, `Formativas`, `Requerimentos`, `Urgente`, `Correio Lixo`, `Outros` (13 values total). `core/models.py:normalize_categoria` aplica um alias map curado (100+ entradas) que tolera output free-form do LLM ("estagio", "Ofícios", etc.) e remapeia legacy categories (Ofícios/Memorandos/Portarias/Informes → Outros) antes da validação Pydantic. `LLMClient.classify_email[_async]` invoca antes do `TypeAdapter.validate_python`.
- **`rag/`** — RAG (Retrieval-Augmented Generation) module for institutional documents. `ingest.py` extracts text from 3,316 PDFs (PyMuPDF + Tesseract OCR fallback for scanned docs), chunks with legal-document-aware separators (LangChain), embeds with `multilingual-e5-large` (sentence-transformers), and indexes in LanceDB (34,285 chunks, 99.2% coverage). Supports `--ocr-only` for re-ingesting previously empty PDFs. `retriever.py` provides semantic search with metadata filters (conselho, tipo). `advanced/raptor.py` provides hierarchical RAPTOR indexing (GMM clustering + recursive summarization) with collapsed tree retrieval — opt-in feature, ativo apenas se a tabela `ufpr_raptor` foi construída via CLI. RAG context is automatically injected into the LLM classification pipeline. Store shared via Google Drive (`RAG_STORE_DIR=G:/Meu Drive/ufpr_rag/store`).
...
- Current branch `feat/marco-i-unified` — Marco I+II+III unified implementation (LangGraph + RAPTOR + Gmail + model cascading + Neo4j GraphRAG).
